...
- Added the missing `coursenamequizname` string. `templates/proctoring_summary.mustache` asked for it as the heading over the combined course-and-quiz name column, it was never defined in `lang/en`, so Moodle would have rendered the literal `[[coursenamequizname]]` there. It now reads "Course / quiz name", which is what the column holds: a course row per course and its quizzes indented under it. Pre-existing since the plugin was first customised; the template is not currently rendered by `proctoringsummary.php`, which builds its table with `flexible_table`, so nobody had seen the broken heading.
