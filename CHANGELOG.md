...
# v1.10.4
- **Sharper ID document captures on phones and high-resolution webcams.** The still photo taken through the camera's photo pipeline was requested with no settings, so most devices returned it at the video stream's resolution and the photo pipeline bought nothing. The capture now asks the camera for its largest still (capped at a 4096-pixel long edge, so a 50 MP phone sensor cannot exhaust the tab's memory) and falls back to the default photo, then to a video frame, when the camera refuses. Only Chrome and Edge expose the photo pipeline; Firefox and Safari keep grabbing a video frame.
- The cropped ID image was then scaled down to at most 1920 pixels wide, which threw away the detail a larger still provides. The cap is now 2560 pixels: fine print on a card stays legible for OCR well past 1920, and a cropped JPEG at that width stays comfortably inside the 8 MB the server accepts. Neither change helps a 720p or 1080p laptop webcam, where the sensor is the limit and holding the card closer to the camera is what improves the read.
