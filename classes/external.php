...
class quizaccess_proctoring_external extends external_api {
    ...
    private static function call_id_verification_endpoint(
        ...
        [$images, $fits, $sizenote] = \quizaccess_proctoring\local\id_verification_payload::fit(
            $images,
            strlen($envelope)
        );
        if ($sizenote !== '') {
        ...
        if (!$fits) {
            ...
            debugging(
                'quizaccess_proctoring ID verification: request body still over '
                    . \quizaccess_proctoring\local\id_verification_payload::MAX_PAYLOAD_BYTES
                    . ' bytes after re-encoding; not sent',
                DEBUG_DEVELOPER
            );
